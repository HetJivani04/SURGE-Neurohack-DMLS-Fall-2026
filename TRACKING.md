...
| 2026-09-20 | Phase2 lead | N=83 REAL scale-up | superseded (see next row) | Manifest 195/112/**83 two-run**; freeze_n83 + β83=**28.438** (vs β49=29.189); data_hash `4e6703055921b762…`. First attempt: artifacts `73533e35` covered **49/83** → `region_emd_procrustes` fallback; posteriors later completed by artifacts `9d7dab12` (83/83). **SUPERSEDED by the completion row below** — N=49 PRIMARY designation retired (kept as replication). |
| 2026-09-20 | Phase2 lead | **N=83 REAL scale-up COMPLETE** | done | 83/83 strict two-run, artifacts `10_ours_full__9d7dab12__20260920T140613Z`, path `posterior_shrink_tau_gated`. **`task_sota_reliability` EARNED:** entropy-λ rel **0.6975** vs 0.6455/0.6461/0.6215; 10k Δrel **+0.0519 [0.0480,0.0558] / +0.0513 [0.0472,0.0552] / +0.0759 [0.0705,0.0814]** (all CIs exclude 0); τ-λ variant 0.6577 (+0.0122/+0.0116/+0.0362). **Ident NOT claimed** — same-map ident protocol map-invariant (fitted/permuted/Haar-RANDOM all 1.0, 83/83; identity 0.9157, single common map 0.9036). Mechanism: `|g(θ)|²=1−2λ(1−λ)(1−cosθ)=cos²(θ/2)` at λ=1/2 (unique maximally-filtering; λ-sweep peaks 0.50). Controls: Haar 0.6454 / permuted 0.6708 / single common map 0.6919 (**89% of gain = template-directed denoising**, subject-specific +0.0056). Group REML n_eff **61.5718 < S=83** (ci_ratio 1.1473; machinery check under unit norm). Gap: τ_φ 0.0345; entropy ambiguity AUROC 1.00. **N=83 PRIMARY.** |

## Current phase

**Phase 2 — REAL SOTA statistical closer COMPLETE at N=83 PRIMARY. Official write-up: `trajot/reports/RESULTS.md`. Stats: `trajot/results/tables/REAL_sota_stats_n83_entropy.{json,md}` (independent replication of `REAL_sota_stats_n83_posterior_entropy.{json,md}`; τ-λ variant `REAL_sota_stats_n83_posterior_tau.{json,md}`). Scale-up summary: `trajot/results/tables/REAL_n83_gap_sota_summary.md`; regenerated table `REAL_n83_gap_sota.{json,md}`. Mechanism evidence: `docs/compose/reports/surge-mathematical-analysis.md`.**

### N=83 completion verdict (2026-09-20) — PRIMARY

- **Task SOTA claim: `task_sota_reliability` EARNED on full N=83.** entropy-λ posterior_shrink (run `10_ours_full__9d7dab12…`, path `posterior_shrink_tau_gated`, 83/83 strict two-run, λ_mean 0.5001 [0.4865, 0.5138] row-entropy gate) scan-rescan reliability **0.6975** vs noalign **0.6455** / BrainSync **0.6461** / FUGW **0.6215**; 10k paired bootstrap Δrel **+0.0519 [0.0480, 0.0558] / +0.0513 [0.0472, 0.0552] / +0.0759 [0.0705, 0.0814]** — all CIs exclude 0. τ-λ variant: **0.6577**, Δrel **+0.0122 / +0.0116 / +0.0362**, also all CIs exclude 0. N=49 replication holds (+0.043/+0.043/+0.065).
- **Identification: NOT claimed (withdrawn).** Control discovery: the per-subject same-map ident protocol is **map-invariant** — fitted, permuted (wrong-subject), and Haar-RANDOM per-subject maps all give **1.0 (83/83)**; identity 0.9157, single common map 0.9036. Ident is reported only as identity preservation (non-collapse), never as superiority. This supersedes any earlier ident claim, including the retracted draft `task_sota_ident_and_reliability`.
- **Mechanism (derived + verified to ≤1e-14):** `T_λ(C)=(1−λ)C+λQᵀCQ` acts on `vec(C)` as `M_λ=(1−λ)I+λR` with `R=Qᵀ⊗Qᵀ` orthogonal; attenuation `|g(θ)|²=1−2λ(1−λ)(1−cosθ)`, which at λ=1/2 equals `cos²(θ/2)` — the **unique maximally-filtering interior point**; λ=0,1 correlation-neutral. λ-sweep on real data peaks exactly at **0.50**.
- **Mechanism controls (reliability @ λ=0.5, real N=83 cache):** raw **0.6455**; Haar-random maps **0.6454** (zero gain — not generic smoothing); permuted fitted maps **0.6708** (≈half the gain); single common template map **0.6919** → **89% of the +0.0519 gain is template-directed denoising** using no subject-specific run-1 information; per-subject fitted **0.6975** → subject-specific increment **+0.0056**. Energy: shared component 29.9% in passband vs 19.0% for run-difference.
- **Gap columns (unclaimed object filled):** per-subject τ_φ (mean **0.0345**); identifiability via posterior row-entropy — AUROC **1.00** on planted-ambiguous synthetic (ambiguous 0.976 vs sharp 0.204 nats); baselines emit null for all these columns.
- **Group REML (all 83 subjects):** n_eff **61.5718** (min 50.311) **< S=83**, ci_ratio 1.1473, no NaN; degenerate-node limit fix (n_eff=#{u=inf} for τ²+σ²=0 nodes, verified by ε-perturbation). Honest caveat: under the stored unit subject-map normalisation this is a machinery check, not a scientific estimate.
- **Competitor rows (N=83 table):** BrainSync rel 0.6461 / ident 0.9036; FUGW rel 0.6215 (collapsed); conn_srm **invalid** (identity collapse, ident 0.024, gain +0.386); point procrustes / ablated rel 0.6483 / ident 0.8434 (hierarchical posterior not driving map).
- **Superseded history (honest ledger — NOT the primary claim):** first attempt artifacts `73533e35` covered 49/83 → fallback `region_emd_procrustes` (Δrel +0.0028/+0.0022/+0.0268); artifact-subset (N=49, β83) +0.0434/+0.0425/+0.0648. Retained in `REAL_n83_gap_sota_summary.md` and `REAL_sota_stats_n83*.md`; N=49 PRIMARY designation retired (kept as replication).
...
| 2026-09-20 | Phase2 lead | N=83 completion + ident withdrawal + mechanism theorem | done | Artifacts `9d7dab12` cover **83/83** → `posterior_shrink_tau_gated` active. `task_sota_reliability` **earned**: rel 0.6975 vs 0.6455/0.6461/0.6215, 10k CIs exclude 0 (Δ +0.0519/+0.0513/+0.0759); τ-λ 0.6577 (+0.0122/+0.0116/+0.0362). **Ident NOT claimed** — same-map protocol map-invariant (fitted/Haar/permuted all 1.0; identity 0.9157, common map 0.9036). Mechanism `cos²(θ/2)` at λ=½; controls Haar 0.6454 / permuted 0.6708 / common map 0.6919 (89% of gain; +0.0056 subject-specific). Group n_eff 61.57<83. Supersedes fallback ledger (`73533e35`, 49/83; subset +0.0434/+0.0425/+0.0648). |
