...
class FUGW(Baseline):
    """CPU-only placeholder adapter for future FUGW integration.

    Stage-1 contract: valid baseline interface that preserves inputs while reporting
    explicit notes that full transport is not yet enabled in this branch.
    """
    ...
    def fit(self, connectomes: np.ndarray, cfg: Mapping[str, Any]) -> "FUGW":
